...
public class OrderDetailDao implements ICommonDao<OrderDetail>, OrderDetailCommonDao {
    ...
    public boolean add(OrderDetail orderDetail) {
        int check = 0;
        String query = "INSERT INTO order_detail (order_id,status_order_detail_id, product_id, "
...
//        int[] arr = {};
//        String query = "UPDATE order_detail SET id =? WHERE order_id = ? AND deleted =0";
//
//        try (Connection con = SQLServerConnection.getConnection(); // mở kết nối đến DB
//                PreparedStatement ps = (con != null) ? con.prepareStatement(query) : null;) {
//            if (ps != null) {
...
//    }
    ...
    // update status
    public boolean updateStatusByOrderID(int orderDetailID, int statusID) {
        int[] arr = {};
        String query = "UPDATE order_detail SET status_order_detail_id =? WHERE order_id = ? AND deleted =0";

        try (Connection con = SQLServerConnection.getConnection(); // mở kết nối đến DB
                PreparedStatement ps = (con != null) ? con.prepareStatement(query) : null;) {
            if (ps != null) {
                ps.setObject(1, statusID);
                ps.setObject(2, orderDetailID);
                ps.addBatch();
            }
            arr = ps.executeBatch();
        } catch (Exception e) {
