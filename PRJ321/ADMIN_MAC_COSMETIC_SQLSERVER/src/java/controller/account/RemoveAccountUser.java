 ...
 * @author HangNT
 */
@WebServlet(name = "RemoveAccountUser", urlPatterns = {"/remove-user"})
public class RemoveAccountUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            int id = Validate.getValidateID(request.getParameter("idUser"));
            if (id > 0) {
                Account account = new AccountService().getOne(id);
                int accountDetailID = account.getAccountDetalId();
//                AccountDetail accountDetail = new AccountDetailService().getOne(accountDetailID);
                // remove
                boolean removeAccount = new AccountService().delete(id);
                boolean removeAccountDetail = new AccountDetailService().delete(accountDetailID);
                if (removeAccount && removeAccountDetail) {
