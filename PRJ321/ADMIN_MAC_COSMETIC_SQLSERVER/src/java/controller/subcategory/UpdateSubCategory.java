 ...
 * @author HangNT
 */
@WebServlet(name = "UpdateSubCategory", urlPatterns = {"/update-sub"})
public class UpdateSubCategory extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String nameSub = request.getParameter("txtNewSub");
            int id = Validate.getValidateID(request.getParameter("idUpdate"));
            int categoryID = Validate.getValidateID(request.getParameter("categoryName"));
            if (categoryID > 0&&id>0) {
                SubCate subCate = new SubCateService().getOne(id);
                subCate.setSubCateName(nameSub);
                subCate.setCategoryId(categoryID);
                boolean updateSub = new SubCateService().update(id, subCate);
                if (!updateSub) {
                    request.setAttribute("messUpdate", "Can't update");
                }
