...
public class Validate {
    ...
    private final static String regexName = "[a-z A-Z]+";
    private final static String regexPassword = "^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$";
    private final static String regexBookId = "[a-zA-Z]{2}\\d{4}";
    ...
    public static String getBookID(JTextField jTextField) {
        String name = jTextField.getText();
        if (name.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Please enter full information!!!");
        } else {
